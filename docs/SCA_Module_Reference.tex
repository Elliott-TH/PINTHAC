...
\paragraph{What it would cost in accuracy.} A table is an approximation, and near the
pseudocritical point $c_p$ has a sharp peak that a coarse grid will clip. \code{rod.py}'s
grid is 3000 points over 710~K, about 0.24~K spacing, which resolves the peak well.
Whether the same is adequate for the annular solver has to be \emph{measured}, not
assumed, by running a case both ways and comparing --- which is the only honest way to
introduce this and is what a toggle is for.

\paragraph{What the toggle should probably look like.} \code{scw\_table.py}'s 2-D $(T,P)$
form is the more capable option, and it would let the annular solver pick up local
pressure the way \code{lut.py} does. \code{rod.py}'s 1-D form is simpler, already proven,
already on GPU, and matches the annular solver's fixed-pressure assumption exactly. The
1-D form is the smaller, safer step; the 2-D form is a capability upgrade as well as a
speedup. Neither has been implemented, and no accuracy comparison has been run.
