"""Permanent tests for pinthac.ml.losses
...
# Every tensor below is built at float64 explicitly.
DT = torch.float64


def test_coolant_energy_residual_h_zero_when_balance_holds():
    mdot = torch.tensor([0.3, 0.5, 1.2], dtype=DT)
    qp = torch.tensor([1.0e4, 2.0e4, 3.0e4], dtype=DT)
...
def test_coolant_energy_residual_T_matches_h_form_via_cp():
    mdot = torch.tensor([0.3, 0.5, 1.2], dtype=DT)
    cp = torch.tensor([5200.0, 5300.0, 5400.0], dtype=DT)
    dT_dz = torch.tensor([10.0, -5.0, 3.0], dtype=DT)
    qp = torch.tensor([1.0e4, 2.0e4, 3.0e4], dtype=DT)
...
def test_normalized_residual_loss_is_mean_squared_ratio():
    res = torch.tensor([2.0, -4.0, 6.0], dtype=DT)
    scale = torch.tensor([2.0, 2.0, 2.0], dtype=DT)
...
def test_residuals_stay_differentiable():
    ...
    mdot = torch.tensor(0.5, dtype=DT, requires_grad=True)
    dh_dz = torch.tensor(200.0, dtype=DT, requires_grad=True)
    qp = torch.tensor(1.0e4, dtype=DT, requires_grad=True)
    res = losses.coolant_energy_residual_h(mdot, dh_dz, qp)
    loss = losses.normalized_residual_loss(res, torch.tensor(1.0e4, dtype=DT))
    loss.backward()
